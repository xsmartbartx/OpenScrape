...
$('#robot-form').onsubmit = async (event) => { event.preventDefault(); try { const id = $('#robot-id').value; const rowSelector = $('#row-selector').value.trim(); const paginationUrlTemplate = $('#pagination-url-template').value.trim(); const body = { name: $('#name').value, startUrl: $('#start-url').value, description: $('#description').value, fields: parseFields($('#fields').value), paginationUrlTemplate: paginationUrlTemplate || null, maxPages: Number($('#max-pages').value), rowSelector: rowSelector || null, maxRows: Number($('#max-rows').value), respectRobotsTxt: $('#respect-robots').checked }; await api(id ? `/api/robots/${id}` : '/api/robots', { method: id ? 'PUT' : 'POST', body: JSON.stringify(body) }); $('#robot-dialog').close(); await refresh(); } catch (error) { $('#form-error').textContent = error.message; } };
